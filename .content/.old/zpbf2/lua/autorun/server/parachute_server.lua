...
if !table.HasValue(tags, "BF2 ZParachute") then 
	table.insert(tags, "BF2 ZParachute")
	table.sort(tags)
...
if v.EndParaTime and CurTime() >= v.EndParaTime and v:OnGround() or v:WaterLevel(1) then --flag
v.EndParaTime = nil
v.Parachuting = false
umsg.Start("PARAIsIn",v)
umsg.Bool(v.Parachuting)
...
v:EmitSound("npc/combine_soldier/zipline_clip2.wav")
//local ParaAB = ents.Create("parachute_abandon")
local ParaAB = ents.Create("92_bf2_zchute_land")
ParaAB:SetOwner(v)
