...
function ParachuteKey(ply, key)
	...
	local Para = ents.Create("92_bf2_zchute_decor")
	Para:SetOwner(ply)
	Para:SetPos(ply:GetPos() + ply:GetUp()*10 + ply:GetForward()*10) -- 100 10
...
umsg.End()
v.FlarePara = 1
v:ViewPunch(Angle(16.5,0,0))
v:EmitSound("npc/combine_soldier/zipline_clip2.wav")
local ParaAB = ents.Create("92_bf2_zchute_abandon")
ParaAB:SetOwner(v)
ParaAB:SetPos(v:GetPos() + v:GetUp()*50 + v:GetForward()*10) -- 100 10
...
local ParaAB = ents.Create("92_bf2_zchute_abandon")
ParaAB:SetOwner(v)
ParaAB:SetPos(v:GetPos() + v:GetUp()*50 + v:GetForward()*10) --115 10
ParaAB:SetAngles(v:GetAngles())
ParaAB:Spawn()
end
...
local ParaAB = ents.Create("92_bf2_zchute_abandon")
ParaAB:SetOwner(v)
ParaAB:SetPos(v:GetPos() + v:GetUp()*50 + v:GetForward()*10)
ParaAB:SetAngles(v:GetAngles())
ParaAB:Spawn()
end
