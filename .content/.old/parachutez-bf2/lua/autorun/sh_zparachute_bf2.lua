...
sound.Add({	["name"] = "VNT_ParachuteZ_BF2_Deploy" ,
	...
	["level"] = 75 ,
	["pitch"] = { 105 , 110 } ,
	["sound"] = { "v92/bf2/vehicles/parachute_deploy.wav" }
})
...
sound.Add({	["name"] = "VNT_ParachuteZ_BF2_Idle" ,
	...
	["level"] = 75 ,
	["pitch"] = { 100 } ,
	["sound"] = { "v92/bf2/vehicles/parachute_ride_loop.wav" }
})
		...
		( GetConVarNumber("vnt_parachutez_sv_mode") == 2 and !table.HasValue( { "superadmin" } , ply:GetNWString( "usergroup" ) ) ) then
			...
			if ( key == IN_JUMP and ply:IsValid( ) and ply:Alive( ) ) and not ( ply:GetMoveType( ) == MOVETYPE_NOCLIP or ply:InVehicle( ) or ply:OnGround( ) or ply.Parachuting == true or ply:GetVelocity( ).z > -600 ) then
				...
				-- if CLIENT then
					...
					LoopingSound = CreateSound( ply , "v92/bf2/vehicles/parachute_ride_loop.wav" )
