...
if GetConVarNumber( "vnt_debug_prints" ) != 0 then	print("ZParachute Redux - Server File Loading...")	end

-- Parachute Functions 'n Shit

function ParachuteKey(_P, key)
	...
	if GetConVarNumber( "vnt_debug_prints" ) >= 1 then	print( "PAIR A SHOES!" )	end
end
...
function ParachuteThink()
	for k,_P in pairs (player.GetAll()) do
		if _P.Parachuting == true then
			...
			if _P.EndParaTime and CurTime() >= _P.EndParaTime and _P:OnGround() then --flag
